...
const AdminDashboard = () => {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      const groupedData = allDates.map((date) => {
        ...
        const transactionsOnDate = transactionResponse.data.filter(
        ...
        const statusTotals = transactionsOnDate.reduce((acc, transaction) => {
          acc[transaction.status] =
            (acc[transaction.status] || 0) + transaction.amount;
          return acc;
        }, {});
  ...
  const exportToExcel = () => {
    ...
    titleWs["A2"].s = {
      font: { italic: true, sz: 12, color: { rgb: "808080" } },
    };
  ...
  const columnChartConfig = {
    ...
    color: (status) => {
      ...
      return colorMap[status]; // Mặc định màu xanh nếu không có trong bảng
    },
        ...
        <Col span={8}>
          ...
          <Card className="report-card">
            ...
            {userReports.map((report, index) => (
              <div key={index}>
                <p>
                  Sender Wallet {report.senderWallet}: {report.totalAmount} đ
                </p>
              </div>
            ))}
          </Card>
